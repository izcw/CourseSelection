...
<el-table-column fixed="right" label="操作" width="180">
    <template #default="scope">
        ...
        <el-table :data="showarrData" :default-sort="{ prop: 'date', order: 'descending' }" style="width: 100%">
            <el-table-column prop="courseName" fixed="left" label="课程名称" />
            ...
            <el-table-column prop="teacher.teacherName" label="老师" />
            <el-table-column prop="beginTime" label="选课开放时间" width="180" />
            <el-table-column prop="endTime" label="选课结束时间" width="180" />
            <el-table-column prop="courseType" label="是否必修">
                <template #default="scope">
...
onMounted(async () => {
    studentInfo.value = await store.getters['user/info'][0]
...
const GetviewcoursetList = () => {
    getviewcoursetList().then(res => {
        if (res.code !== 200) {
            console.log("课程信息-获取不到数据");
        } else {
            showarrData.value = res.data
...
const showMyselect = async () => {
    ...
    await getSelectcoursesList({ studentCode: studentInfo.value.studentCode }).then(res => {
        if (res.code !== 200) {
            console.log("我的选课信息-获取不到数据");
        } else {
            myselectArr.value = res.data
            console.log(myselectArr.value, 'selectedCourses');
        }
    });
}
